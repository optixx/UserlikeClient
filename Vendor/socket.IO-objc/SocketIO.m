...
#define HANDSHAKE_URL_SSL @"https://%@:%d/socket.io/1/?t=%d"
#define SOCKET_URL_SSL @"wss://%@:%d/socket.io/1/websocket/%@"
...
- (void) connectToHost:(NSString *)host onPort:(NSInteger)port withSSL:(BOOL)ssl {
    ...
    if (!_isConnected && !_isConnecting) 
    {
        ...
        NSString *s; 
        if (ssl) {
            s = [NSString stringWithFormat:HANDSHAKE_URL_SSL, _host, _port, rand()];
        } else {
            s = [NSString stringWithFormat:HANDSHAKE_URL, _host, _port, rand()];
        }
...
- (void) openSocket
{
    ...
    NSString *url;
    if (_isSSL) {
        url = [NSString stringWithFormat:SOCKET_URL_SSL, _host, _port, _sid];
    } else {
        url = [NSString stringWithFormat:SOCKET_URL, _host, _port, _sid];
    }
...
- (void) webSocket:(WebSocket *)ws didFailWithError:(NSError *)error 
{
    ...
    [_delegate socketIOconnectionFailed:self];
